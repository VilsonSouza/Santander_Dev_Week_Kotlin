...
@Composable
fun MenuItems(modifier: Modifier=Modifier, features: List<Feature> = emptyList()) {
    LazyRow(modifier = modifier, horizontalArrangement = Arrangement.spacedBy(Spacing_2)){
        items(features){ feature ->
            AppCard (modifier = Modifier.size(width=110.dp,height=96.dp) ){
                Column(modifier = Modifier.fillMaxSize(), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
                    SvgImage(url = feature.icon)
                    Text(text = feature.description,modifier= Modifier.padding(top= Spacing_1))
                }
            }
        }
    }
